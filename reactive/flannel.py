...
def configure_network(etcd):
    ...
    """
    data = json.dumps({"Network": config("cidr"), "Backend": {"Type": "vxlan"}})
    cmd = "etcdctl "
...
def ensure_etcd_connections():
    ...
    connection_changed = data_changed(
        "flannel_etcd_connections", etcd.get_connection_string()
    )
    cert_changed = data_changed("flannel_etcd_cert", etcd.get_client_credentials())
